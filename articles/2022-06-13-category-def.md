...
この言葉を、参考記事にある圏の具体例を使いつつ，自分なりに説明をしてみます．
...
文字列が三つあっても、合成は二つずつしかできない。

まずは、`りんご`と`ごりら`を合成して、`りんごりら`をつくる。
その後、`りんごりら`と`らっぱ`を合成して、`りんごりらっぱ`をつくる。

実は、合成の順番はもう一つある。
先に、`ごりら`と`らっぱ`を合成して、`ごりらっぱ`をつくる。
そのあと、`りんご`と`ごりらっぱ`を合成して、`りんごりらっぱ`をつくる。

先にどちらを合成しても、`りんごりらっぱ`という同じひらがなの文字列になった．

## ✅合成のルール　その２

三つの文字列の合成で，どちらを先に合成したとしても，`りんごりらっぱ`という合成結果は変わらなかった。
